...
/**
 * This Class is the Log4j Wrapper we can use this to change the implementation
 * of logging without affecting the rest of the logging code. We have used the
 * adaptor pattern here to encapsulate the the Log4j logging behaviour
 * 
...
public class Logger {
	...
	/**
	 * This method logs the message string containing the arguements on to the
	 * specified log level.
	 * 
	 * @param level
	 *            The level to log.
	 * @param msg
	 *            The messages to be loged containing the placeholders for
	 *            arguements like {index}.
	 * @param args
	 *            The arguements of the message string.
	 */
	...
	public void LogPrecise(LogLevel level, String srcClass, String srcMethod, Throwable excep, String msg,
			Object... args) {

		if (this.isLoggable(level)) {

			this.log(level, String.format("[%1$s].%2$s method, Log Msg: %3$s.%4$s Exception: %5$s.", srcClass,
					srcMethod, String.format(msg, args), System.getProperty("line.separator"), excep), excep);
		}
	}

	public void LogPrecise(LogLevel level, String srcClass, String srcMethod, String msg, Throwable exception) {

		this.log(level, String.format("%1$s.%2$s method log: %3$s.%4$s, Exception: %5$s.", srcClass, srcMethod, msg,
				System.getProperty("line.separator"), exception), exception);
	}
	...
	public void LogPrecise(LogLevel level, Throwable excep, String msg, Object... args) {
		...
		try {
			...
			throw new LogException(ex, "Error traversing caller method in LogP method, Logger: %1$s",
					this.logger.getName());
		}
	}
	...
	public void LogPrecise(LogLevel level, String msg, Throwable exception) {
		...
		try {
			...
			throw new LogException(ex, "Error traversing caller method at LogP method, Logger: %1$s",
					this.logger.getName());
		}
	}
	...
	public static void main(String[] args) {
		...
		logt.LogPrecise(LogLevel.Info, "myClass", "myMethod", new NullPointerException("null found"),
				"Another Message %1$s having args ones %2$s : another args %3$s.", "message", "arg1", "arg2");
	}
}
