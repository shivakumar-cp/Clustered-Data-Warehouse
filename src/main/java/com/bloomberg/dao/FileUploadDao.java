...
public class FileUploadDao implements IFileUploadDAO {
	...
	public boolean fileExists(String fileName) {
		...
		return ((Long) session.createQuery("select count(*) from ValidDeal where fileName=:fileName")
				.setParameter("fileName", fileName).uniqueResult()).intValue() > 0 ? true : false;
	}
	 ...
	 * @param entities
	 *            ValidBulkDeal Collection
	 * @param accumulativeDeals
	 *            List of AccumulativeDeal
	 * 
	 */
	@Override
	public <T extends ValidDeal> Collection<T> bulkValidSave(Collection<T> entities,
			List<AccumulativeDeal> accumulativeDeals) {
	...
	public void saveAccumulativeDeals(List list) throws HibernateException {
		...
		while (it.hasNext()) {
			...
			List<AccumulativeDeal> deals = session
					.createQuery("from AccumulativeDeal where oderingCurrency=:oderingCurrency")
